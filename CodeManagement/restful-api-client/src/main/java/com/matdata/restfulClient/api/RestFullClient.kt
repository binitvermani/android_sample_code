...
import com.matdata.restfulClient.api.extension.handleException
import com.matdata.restfulClient.api.extension.log
import com.matdata.restfulClient.api.utils.SingletonHolder
                ...
                if (response.isSuccessful) {
                    ...
                    if(response.code() == 404){
                        ...
                        if (responseUrl.contains("?"))
                            responseUrl = responseUrl.substring(0, responseUrl.indexOf("?"))
                        log("url_new", responseUrl)
                        mSyncEventListener.onSyncSuccess(response.code(), response.message(), responseUrl, "")
                    }
