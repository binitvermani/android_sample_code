...
class VoterSearchFragment : BaseFragment(), ClickHandler, BaseAdapter.OnItemClickListener {

    private var binding: FragmentVoterSearchBinding? = null
    private var adapter : VoterSearchAdapter? = null

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        if(binding == null) {
            binding = FragmentVoterSearchBinding.inflate(inflater, container, false)
        }
        return binding!!.root
    }
    ...
    private fun setAdapter() {
        adapter = VoterSearchAdapter(baseActivity!!)
